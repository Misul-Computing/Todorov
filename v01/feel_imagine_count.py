...
def make_occ_count(batch, strip_len, n_occluded, p_stay, touch, device="cpu", generator=None, fake=False):
    ...
    if n_occluded > 0:
        occ = torch.rand(batch, strip_len, generator=generator).argsort(dim=1)[:, :n_occluded]
...
def make_fn(args, p_stay, touch, fake, n_occluded):
    g = torch.Generator().manual_seed(args.seed + 1)

    def mk(b, device):
        return make_occ_count(b, args.strip_len, n_occluded, p_stay, touch, device, generator=g, fake=fake)
    return mk
...
def run_arm(mk, args, device):
    ...
    leak = sanity.causal_no_future_leak(model, 5 + 2 * args.strip_len, 4, args.probe_len, device)
...
def main():
    ...
    ap.add_argument("--imag_min", type=float, default=0.6)
    ap.add_argument("--device", default="cpu")
    ap.add_argument("--seed", type=int, default=0)
    args = ap.parse_args()
    device = args.device
    arms = (
        ("blind", dict(p_stay=args.p_stay, touch=False, fake=False, n_occluded=args.n_occluded)),
        ("observed_full", dict(p_stay=args.p_stay, touch=True, fake=False, n_occluded=0)),
        ("occ_structured", dict(p_stay=args.p_stay, touch=True, fake=False, n_occluded=args.n_occluded)),
        ("occ_random", dict(p_stay=None, touch=True, fake=False, n_occluded=args.n_occluded)),
        ("fake", dict(p_stay=args.p_stay, touch=True, fake=True, n_occluded=args.n_occluded)),
    )
    print(f"occ_count strip={args.strip_len} occ={args.n_occluded} p_stay={args.p_stay} "
    ...
    for name, kw in arms:
        ...
        res[name] = {"token": ev["token_acc"], "exact": ev["exact_acc"], "loss": loss, "causal_ok": ok}
        print(f"  {name:14s} acc={ev['token_acc']:.3f} loss={loss:.3f} causal_ok={ok} [{time.time() - t:.1f}s]", flush=True)
    s = res["occ_structured"]["token"]
    r = res["occ_random"]["token"]
    o = res["observed_full"]["token"]
    imagines = (s - r >= args.imag_margin and s >= args.imag_min and o >= 0.85
                and res["blind"]["token"] < 0.4 and res["fake"]["token"] < 0.4)
    print(f"  IMAGINATION {'YES' if imagines else 'NO'} (occ_structured {s:.3f} - occ_random {r:.3f} "
          f"= {s - r:.3f}, observed_full {o:.3f}, blind {res['blind']['token']:.3f}, "
          f"fake {res['fake']['token']:.3f})", flush=True)
