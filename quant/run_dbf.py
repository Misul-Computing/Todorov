"""Test DBF at 1-2 bpw on real model."""
import os
...
from quant.dbf import dbf_quantize_model
...
def main():
    ...
    for label, bpw, n_iters in [("dbf_1bpw", 1.0, 20), ("dbf_2bpw", 2.0, 20)]:
        restore(model, snap)
        tq = time.time()
        try:
            log = dbf_quantize_model(model, bpw, n_iters=n_iters,
            ...
            avg_bpw = sum(e["bpw"] for e in log if not e.get("skipped")) / \
                      max(1, sum(1 for e in log if not e.get("skipped")))
            ppl = ppl_wt2(model, tok, device=dev)
            print(f"[{label:25s}] ppl={ppl:.4f}  d={ppl - base:+.4f}  "
                  f"bpw={avg_bpw:.3f}  recon={avg_recon:.4f}  ({time.time() - tq:.1f}s)",
                  flush=True)
            results.append({"cfg": label, "ppl": ppl, "ppl_delta": ppl - base,
                            "avg_bpw": avg_bpw, "avg_recon": avg_recon})
    ...
    out_dir = "/kaggle/working/runs/dbf" if os.path.isdir("/kaggle/working") else \
              os.path.join(os.path.dirname(os.path.abspath(__file__)), "runs", "dbf")
    os.makedirs(out_dir, exist_ok=True)
    with open(os.path.join(out_dir, "summary.json"), "w") as f:
        json.dump({"experiment": "DBF 1-2 bit", "model": MODEL_ID,
