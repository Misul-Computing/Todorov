"""Test learned codebooks at 1 bpw: AQLM-style d=8/256 + d=4/16 + outlier separation.

AQLM achieves 7.85 PPL at 1 bpw on LLaMA-2-7B with d=8, 256 entries, learned codebook + fine-tuning.
We test the k-means-only version (no gradient refinement) to see the gap.
"""
import os
...
def main():
    ...
    configs = [
        ("scalar_b2",                    "fullrot_whlm",        2, 128),
        ("vq_datafit_d4_b2",             "fullrot_vq:datafit:2:1", 2, 128),
        ("vq_datafit_d4_b2_out1",        "fullrot_vq:datafit:2:1:1.0", 2, 128),
        ("vq_gaussian_d8_b1",            "fullrot_vq:gaussian8:1:1", 1, 128),
        ("vq_datafit_d8_b1",             "fullrot_vq:datafit8:1:1", 1, 128),
        ("vq_datafit_d8_b1_out1",        "fullrot_vq:datafit8:1:1:1.0", 1, 128),
        ("vq_datafit_d8_b1_out2",        "fullrot_vq:datafit8:1:1:2.0", 1, 128),
        ("vq_datafit_d8_b1_out5",        "fullrot_vq:datafit8:1:1:5.0", 1, 128),
        ("vq_gaussian_d4_b1",            "fullrot_vq:gaussian:1:1", 1, 128),
        ("vq_datafit_d4_b1",             "fullrot_vq:datafit:1:1", 1, 128),
        ("vq_datafit_d4_b1_out1",        "fullrot_vq:datafit:1:1:1.0", 1, 128),
        ("vq_datafit_d4_b2_out2",        "fullrot_vq:datafit:2:1:2.0", 2, 128),
        ("vq_datafit_d4_b1_m2",          "fullrot_vq:datafit:1:2", 1, 128),
        ("vq_datafit_d8_b1_m2",          "fullrot_vq:datafit8:1:2", 1, 128),
    ]
    ...
    out_dir = "/kaggle/working/runs/learned1bit" if os.path.isdir("/kaggle/working") else \
              os.path.join(os.path.dirname(os.path.abspath(__file__)), "runs", "learned1bit")
    os.makedirs(out_dir, exist_ok=True)
    out_path = os.path.join(out_dir, "summary.json")
    with open(out_path, "w") as f:
        json.dump({"experiment": "learned codebooks at 1 bpw",
