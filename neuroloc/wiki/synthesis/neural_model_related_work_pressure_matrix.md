...
1. harden the local 10k responder after it cleared grounded response, refusal, update, targeted replay, branch-state, route-disabled collapse, and shuffled-route controls. keep the work local and test longer grounded dialogue, provenance-preserving rewrites, factor-heldout query forms, fair sparse-read baselines, larger local datasets, and stronger storage-in-local-state formulations before any broader claim.
