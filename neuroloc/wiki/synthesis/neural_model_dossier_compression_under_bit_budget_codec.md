...
the next executable work is not the full model. the tiny local mirror defined in [[neural_model_tiny_mirror_contract_compression_under_bit_budget]] now exists and is documented in [[tests/compression_under_bit_budget_mirror]]. its first learned-codec/source-diagnostic result was negative, exposed a source-observability contract problem, repaired that contract, added sparse-read and matched-budget controls, added distributed evidence, produced an ordinary-split tiny local pass, failed factor-heldout recombination with a shared nonlinear trunk, repaired that specific gate with a factorized structured local codec, cleared a 10k-scale local data-heavy gate across four heldout axes and three smoke seeds, and then added the constrained generated-language bridge in [[tests/language_grounded_state_density_mirror]]. the next local work is hard-profile robustness, less hand-shaped message parsing, and a stronger storage-in-local-state formulation before another broader codec is trained.
