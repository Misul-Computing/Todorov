...
- [[tests/compression_under_bit_budget_mirror]] documents the implemented local dataset, guard, baseline-control, learned-codec, source-diagnostic, bit-accounting, sparse-read baseline, distributed-evidence probe, tiny local learned model, factor-heldout gate, factorized structured local codec, 10k-scale local data-heavy gate, useful-state-density telemetry, and telemetry surface. [[tests/language_grounded_state_density_mirror]] documents the first constrained generated-language bridge, negative parser-resistant token-count gate, parser-supported event-binding baseline, and typed trainable event-binding local-state pass. [[tests/local_v1_language_model]] documents the first dataset-grounded local v1 state-first responder with constrained memory update, targeted replay, random-replay failure, and branch-state controls. [[tests/local_10k_chat_model]] documents the first integrated 10k constrained chat responder with route-disabled and shuffled-route controls. [[tests/local_foundation_neural_model]] documents the first under-10k foundation responder with explicit context, working memory, episodic memory, long-term memory, replay buffer, branch memory, and codec-disabled, replay-disabled, and branch-disabled controls. the shared nonlinear factor-heldout model fails at test joint success 0.03125, while the 9,792-parameter factorized structured local codec clears four heldout factor axes across three smoke seeds at minimum test joint success 1.0, the 3,847-parameter language bridge clears four heldout axes across two smoke seeds, the 8,856-parameter typed trainable event binder clears the randomized symbolic-message gate at minimum heldout joint/state/action success 0.9583333333333334, the 2,720-parameter local v1 router clears its dataset-memory and dialogue controls, the 7,968-parameter local 10k chat responder clears its grounded chat gate with route-control collapse, and the 5,560-parameter foundation responder clears its architecture-boundary gate with route, codec, replay-buffer, and branch-memory control collapse. this is a narrow local symbolic-language, dataset-record, and grounded foundation gate, not arbitrary chat or solved compression. the current bottleneck is larger local data ingestion, factor-heldout query forms, stronger memory ablations, fair sparse-read baselines, and stronger learned binding, not paid compute.
...
harden the foundation responder in [[tests/local_foundation_neural_model]] through larger local dataset ingestion, factor-heldout query forms, zero-working-memory controls, zero-long-term-memory controls, shuffled-code controls, and fair sparse-read baselines, while keeping oracle compression bounds and family-specific controls ahead of any learned-codec or full-model path.
