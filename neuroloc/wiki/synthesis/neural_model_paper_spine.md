...
proof state: partially bounded and not learned. the first oracle compression result and frontier split exist, but they are mixed across families. the first learned codec for `compression_under_bit_budget` fails held-out operation preservation, and the first content-routed sparse-read baseline solves the repaired source-pair smoke task from two legal raw records while exceeding the compact-code budget. matched-budget sparse read fails, and the new distributed-evidence probe is solved by four-record sparse read but fails under the matched compact budget. the operation-preserving compression stack remains unproved.
...
the first executable model experiment is not a full model run. the cellular/local-state gap map, first cellular mechanism dossier, symbolic/oracle contract implementation, first oracle compression analysis, frontier split, `compression_under_bit_budget` proof package, tiny mirror contract, local mirror baseline surface, first negative learned-codec smoke result, source-diagnostic pass, source-observability repair, content-routed sparse-read baseline, matched-budget sparse-read control, and distributed-evidence probe come first. because the first learned codec still overfits train split and does not beat sparse read after the source repair, the next model experiment is not a paid run or full integration. first make oracle-code/learned-decoder generalize on the distributed-evidence slice, then attempt end-to-end learned compact-code operation preservation. full integration and paid compute remain blocked.
...
- the first candidate proof package for `compression_under_bit_budget` has produced a repaired source-observability result, a sparse-read demotion of the source-pair task, a distributed-evidence probe, and a negative learned result: legal visible-source extraction solves the original task, but learned address, payload color, payload position, velocity, action, and learned decoder generalization still fail. the next obligation is learned compact-code operation preservation on distributed evidence or proof-package demotion rather than claim expansion.
...
next, repair the learned-codec failure on the distributed-evidence slice inside the `compression_under_bit_budget` local mirror. source state and action target are legally inferable from non-oracle input, but the source-pair task collapses to shallow sparse read. the next localization target is oracle-code to learned-decoder generalization first, then learned address, payload color, payload position, velocity, action, provenance, and useful bits per successful episode. if the local mirror still needs oracle payload or oracle decoder exposure to pass, or cannot beat sparse read under useful-bit accounting, revise or demote the symbolic contract before any model path.
