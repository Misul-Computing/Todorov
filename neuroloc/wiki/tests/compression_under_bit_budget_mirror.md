...
test type: local tiny-mirror dataset, baseline-control, learned-codec, and diagnostic-localization surface
...
implemented the first local code surface for the accepted `compression_under_bit_budget` family, the first trainable learned-codec smoke result, and the first diagnostic-only failure-localization controls. this is not a passing compression result. it is a controlled negative trainability result: the tiny codec memorizes train split but does not preserve the target operation on held-out episodes.
...
- diagnostic-only controls for learned-code/oracle-decoder, oracle-code/learned-decoder, learned-address/oracle-payload, oracle-address/learned-payload, and provenance-exposed learned codec.
...
commands run on 2026-04-27 after the diagnostic-localization update:
...
- result: 16 passed, 1 known numpy-on-windows warning
- `python neuroloc/simulations/suite_runner.py --suite compression_mirror --profile smoke --output-root .pytest_tmp_diag_448 --write-summary`
...
- result: 323 tests collected, 1 known numpy-on-windows warning
...
- oracle-code/learned-decoder joint success: 0.0
...
the diagnostic controls localize the first failure more sharply. on the smoke profile, the learned address is partly useful, because replacing the payload with oracle values while keeping the learned address rescues joint success to 0.5. the learned payload is not useful: replacing the address with oracle values while keeping the learned payload remains 0.0. the learned decoder also fails on oracle code in this small smoke setting. provenance exposure does not rescue the learned codec. this points first at payload/action/source-state inference and decoder trainability, not at a proved addressable compression mechanism.
...
accepted as the local dataset, baseline-control, first learned-codec smoke surface, and first diagnostic-localization surface for the `compression_under_bit_budget` tiny mirror. the learned result fails the engineering and paper-track gates. the next no-paid work is to revise the tiny mirror around payload/action/source-state inference and learned decoder generalization before any broader mirror, full-model integration, or paid compute.
