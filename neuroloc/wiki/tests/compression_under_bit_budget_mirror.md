...
the factorized structured codec does not prove the full storage/compression stack. it proves one local symbolic repair and one 10k-scale local data-heavy robustness gate across four heldout axes and three smoke seeds. it still needs hard-profile sweeps, other world families, less hand-shaped event pooling, and a stronger local-state storage formulation before the proof package can expand.
