...
SEED = env_int("SOURCE_BLOCK_CODEC_SEED", 941)
FACTS_SMOKE = env_int("SOURCE_BLOCK_CODEC_FACTS_SMOKE", 4096)
FACTS_HARD = env_int("SOURCE_BLOCK_CODEC_FACTS_HARD", 4096)
TRAIN_FACTS_SMOKE = env_int("SOURCE_BLOCK_CODEC_TRAIN_FACTS_SMOKE", 2048)
TRAIN_FACTS_HARD = env_int("SOURCE_BLOCK_CODEC_TRAIN_FACTS_HARD", 2048)
CHUNK_BYTES = env_int("SOURCE_BLOCK_CODEC_CHUNK_BYTES", 32)
DECODER_BITS = env_int("SOURCE_BLOCK_CODEC_DECODER_BITS", 65536)
MANIFEST_DECODER_BITS = env_int("SOURCE_BLOCK_CODEC_MANIFEST_DECODER_BITS", 0)
ORDINARY_BITS_PER_PARAMETER = float(os.environ.get("SOURCE_BLOCK_CODEC_ORDINARY_BITS_PER_PARAMETER", "2.5"))
TARGET_MULTIPLIER = float(os.environ.get("SOURCE_BLOCK_CODEC_TARGET_MULTIPLIER", "600.0"))
CHARGED_CODEC_BASELINE_MULTIPLIER = float(os.environ.get("SOURCE_BLOCK_CODEC_CHARGED_CODEC_BASELINE_MULTIPLIER", "13.941917871967359"))

require_positive("SOURCE_BLOCK_CODEC_FACTS_SMOKE", FACTS_SMOKE)
require_positive("SOURCE_BLOCK_CODEC_FACTS_HARD", FACTS_HARD)
require_positive("SOURCE_BLOCK_CODEC_TRAIN_FACTS_SMOKE", TRAIN_FACTS_SMOKE)
require_positive("SOURCE_BLOCK_CODEC_TRAIN_FACTS_HARD", TRAIN_FACTS_HARD)
require_positive("SOURCE_BLOCK_CODEC_CHUNK_BYTES", CHUNK_BYTES)
require_positive("SOURCE_BLOCK_CODEC_DECODER_BITS", DECODER_BITS)
...
def infer_profile() -> str:
    ...
    if raw in {"smoke", "hard"}:
        return raw
    value = os.environ.get("SOURCE_BLOCK_CODEC_PROFILE", "smoke").strip()
    if value not in {"smoke", "hard"}:
        raise ValueError("SOURCE_BLOCK_CODEC_PROFILE must be smoke or hard")
    return value
...
def candidate_offsets(test_manifest: list[dict[str, Any]]) -> list[tuple[int, int, str]]:
    ...
    for source_id, row in enumerate(test_manifest):
        ...
        for offset in range(start, end + 1, int(CHUNK_BYTES)):
            candidates.append((source_id, int(offset), str(row["name"])))
    return candidates
...
def sample_test_offsets(source_block: bytes, test_manifest: list[dict[str, Any]], count: int, seed: int) -> list[tuple[int, int, str]]:
    ...
    for source_id, offset, source in candidates:
        chunk = source_block[int(offset) : int(offset) + int(CHUNK_BYTES)]
    ...
    raise ValueError("not enough unique source chunks")
...
def key_for(source_id: int, offset: int) -> tuple[int, int]:
    return (int(source_id), int(offset))
...
class SourceBlockExactCodecCell:
    def __init__(self, train_facts: list[dict[str, Any]], test_facts: list[dict[str, Any]], source_block: bytes, manifest: list[dict[str, Any]]) -> None:
        ...
        self.source_offset_routing_used = 1.0
    ...
    def read(
        ...
        source_id, offset = int(key[0]), int(key[1])
        if source_id < 0 or source_id >= len(self.manifest):
            return {"value": "", "provenance": "", "hit": 0}
        block = decompress_block(self.codec_name, self.block_stream)
        self.decompression_count += 1
        if offset < 0 or offset + int(CHUNK_BYTES) > len(block):
            return {"value": "", "provenance": "", "hit": 0}
        row = self.manifest[source_id]
        start = int(row["block_offset"])
        end = start + int(row["length"])
        if not (start <= offset and offset + int(CHUNK_BYTES) <= end):
            return {"value": "", "provenance": "", "hit": 0}
        value = block[offset : offset + int(CHUNK_BYTES)]
        return {"value": value.hex(), "provenance": provenance_for(self.manifest, source_id, offset, value), "hit": 1}


def evaluate_controls(cell: SourceBlockExactCodecCell, facts: list[dict[str, Any]], random_twin: list[dict[str, Any]]) -> dict[str, float]:
    exact = [cell.read(tuple(fact["key"])) for fact in facts]
    twin_reads = [cell.read(tuple(fact["key"])) for fact in random_twin]
...
def accounting(cell: SourceBlockExactCodecCell, fact_count: int, manifest: list[dict[str, Any]]) -> dict[str, float]:
    source_bits = bits_for_cardinality(len(manifest))
    offset_bits = bits_for_cardinality(cell.source_block_len)
    source_offset_bits = int(source_bits + offset_bits)
    codec_selector_bits = bits_for_cardinality(3)
    manifest_cost_bits = int(manifest_bits(manifest) + int(MANIFEST_DECODER_BITS))
    committed_state_bits = int(cell.block_payload_bits + source_offset_bits + codec_selector_bits + int(DECODER_BITS) + manifest_cost_bits)
    ...
    return {
        ...
        "source_offset_bits": float(source_offset_bits),
...
def baseline_metrics(useful_bits: int, fact_count: int, account: dict[str, float]) -> dict[str, float]:
    row_bits = 2 * 32 + int(CHUNK_BYTES) * 8 + 64
    verbatim_bits = int(fact_count * row_bits)
    mph_payload_bits = int(account["block_payload_bits"] + account["source_offset_bits"] + account["decoder_bits"] + account["manifest_bits"])
    return {
...
def run_profile(profile: str, seed: int = SEED) -> dict[str, Any]:
    ...
    cell = SourceBlockExactCodecCell(train_facts, facts, source_block, manifest)
    ...
    product_pass = float(int(controls["exact_retrieval_success"] >= 0.95 and controls["random_label_twin_success"] == 0.0 and controls_collapse == 1.0 and beats_charged_codec == 1.0 and strict_600x_pass == 0.0))
    return {
        "profile": profile,
...
def build_summary(profile: str, seed: int = SEED) -> dict[str, Any]:
    ...
    summary: dict[str, Any] = {
        ...
        "local_100k_source_block_codec_engineering_pass": float(row["product_pass"]),
    }
    for key, value in row.items():
        if key in {"profile"}:
            continue
        summary[f"local_100k_source_block_codec_{key}"] = float(value)
    return summary
...
def main() -> int:
    ...
    metrics_path = output_dir / "local_100k_source_block_codec_metrics.json"
    finished_at = utc_now_iso()
    record = build_run_record(
        simulation_name="local_100k_source_block_codec",
        ...
        seed_numpy=int(SEED),
        n_trials=int(summary["local_100k_source_block_codec_fact_count"]),
        summary=summary,
        statistics={},
        trials=[],
        warnings=[],
        artifacts=[{"name": "local_100k_source_block_codec_metrics.json", "path": metrics_path}],
    )
