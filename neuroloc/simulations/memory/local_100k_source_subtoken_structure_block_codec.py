...
SEED = env_int("SOURCE_SUBTOKEN_STRUCTURE_BLOCK_CODEC_SEED", 9419)
SUBTOKEN_STRUCTURE_HEADER_BITS = env_int("SOURCE_SUBTOKEN_STRUCTURE_BLOCK_CODEC_HEADER_BITS", 896)
TOKEN_LIMIT = env_int("SOURCE_SUBTOKEN_STRUCTURE_BLOCK_CODEC_TOKEN_LIMIT", 120)
MIN_STRICT_IMPROVEMENT = float(os.environ.get("SOURCE_SUBTOKEN_STRUCTURE_BLOCK_CODEC_MIN_STRICT_IMPROVEMENT", "0.048"))
MIN_PAYLOAD_IMPROVEMENT = float(os.environ.get("SOURCE_SUBTOKEN_STRUCTURE_BLOCK_CODEC_MIN_PAYLOAD_IMPROVEMENT", "0.061"))
SOURCE_TOKEN_STRICT_IMPROVEMENT = float(os.environ.get("SOURCE_SUBTOKEN_STRUCTURE_BLOCK_CODEC_SOURCE_TOKEN_STRICT_IMPROVEMENT", "0.03543501930119766"))
SOURCE_TOKEN_PAYLOAD_BITS = float(os.environ.get("SOURCE_SUBTOKEN_STRUCTURE_BLOCK_CODEC_SOURCE_TOKEN_PAYLOAD_BITS", "123088"))

require_positive("SOURCE_SUBTOKEN_STRUCTURE_BLOCK_CODEC_HEADER_BITS", SUBTOKEN_STRUCTURE_HEADER_BITS)
require_positive("SOURCE_SUBTOKEN_STRUCTURE_BLOCK_CODEC_TOKEN_LIMIT", TOKEN_LIMIT)

PROFILES = {
    "smoke": {"target_count": 4, "min_strict_improvement": 0.048, "min_payload_improvement": 0.061},
    "hard": {"target_count": 4, "min_strict_improvement": 0.048, "min_payload_improvement": 0.061},
}
...
def infer_profile() -> str:
    ...
    if raw in {"smoke", "hard"}:
        return raw
    value = os.environ.get("SOURCE_SUBTOKEN_STRUCTURE_BLOCK_CODEC_PROFILE", "smoke").strip()
    if value not in {"smoke", "hard"}:
        raise ValueError("SOURCE_SUBTOKEN_STRUCTURE_BLOCK_CODEC_PROFILE must be smoke or hard")
    return value


def substitute_subtokens(body: bytes, tokens: list[bytes]) -> bytes:
    if not tokens:
        return body
    ...
    while index < len(body):
        ...
        for token in ordered:
            if body.startswith(token, index):
                ...
                break
        if chosen:
            out.append(int(token_ids[chosen]))
            index += len(chosen)
        else:
...
def learned_codec(train_block: bytes, target_block: bytes, token_limit: int = TOKEN_LIMIT) -> dict[str, Any]:
    ...
    substituted_body = substitute_subtokens(bytes(transformed["body_stream"]), tokens)
...
class SourceSubtokenStructurePayloadModule(torch.nn.Module):
    def __init__(self, learned: dict[str, Any]) -> None:
        super().__init__()
        holder = SourceTokenStructurePayloadModule(learned)
        for name, tensor in holder.state_dict().items():
            self.register_buffer(name, tensor.clone())

    @classmethod
    def empty_like(cls, other: "SourceSubtokenStructurePayloadModule") -> "SourceSubtokenStructurePayloadModule":
        learned = {
    ...
    def learned_state(self) -> dict[str, Any]:
        holder = SourceTokenStructurePayloadModule.empty_like(SourceTokenStructurePayloadModule(self._raw_learned()))
        holder.load_state_dict(self.state_dict())
        return holder.learned_state()

    def _raw_learned(self) -> dict[str, Any]:
        return {
            "count_payload": bytes(int(self.count_payload.numel())),
            "body_payload": bytes(int(self.body_payload.numel())),
            "dictionary_payload": bytes(int(self.dictionary_payload.numel())),
            "count_codec_name": "zlib1",
            "body_codec_name": "zlib1",
            "dictionary_codec_name": "none0",
            "line_count": 0,
...
def measure_block(train: bytes, target: bytes, seed: int) -> dict[str, float | str]:
    ...
    module = SourceSubtokenStructurePayloadModule(learned)
    module_reloaded = SourceSubtokenStructurePayloadModule.empty_like(module)
    ...
    learned_payload_bits = int((len(bytes(learned["count_payload"])) + len(bytes(learned["body_payload"])) + len(bytes(learned["dictionary_payload"]))) * 8 + int(SUBTOKEN_STRUCTURE_HEADER_BITS))
    baseline_payload_bits = int(len(baseline_payload) * 8)
    source_token_payload_bits = int(SOURCE_TOKEN_PAYLOAD_BITS)
    ...
    source_token_strict_bits = int(source_token_payload_bits + int(DECODER_BITS) + int(MODEL_HEADER_BITS))
    ...
    random_learned_payload_bits = int((len(bytes(random_learned["count_payload"])) + len(bytes(random_learned["body_payload"])) + len(bytes(random_learned["dictionary_payload"]))) * 8 + int(SUBTOKEN_STRUCTURE_HEADER_BITS))
    ...
    return {
        ...
        "body_subtoken_codec": str(learned["body_codec_name"]),
        ...
        "body_subtoken_codec_family_id": codec_family_rank(str(learned["body_codec_name"])),
        ...
        "source_token_payload_bits": float(source_token_payload_bits),
        "learned_count_delta_payload_bits": float(len(bytes(learned["count_payload"])) * 8),
        "learned_body_subtoken_payload_bits": float(len(bytes(learned["body_payload"])) * 8),
        "learned_dictionary_payload_bits": float(len(bytes(learned["dictionary_payload"])) * 8),
        "learned_subtoken_structure_header_bits": float(SUBTOKEN_STRUCTURE_HEADER_BITS),
        "learned_payload_bits": float(learned_payload_bits),
        "best_standard_strict_bits": float(baseline_strict_bits),
        "source_token_strict_bits": float(source_token_strict_bits),
        ...
        "source_token_strict_improvement_baseline": float(SOURCE_TOKEN_STRICT_IMPROVEMENT),
        "strict_improvement_delta_over_source_token": strict_improvement - float(SOURCE_TOKEN_STRICT_IMPROVEMENT),
        "payload_improvement_delta_over_source_token": float(source_token_payload_bits - learned_payload_bits) / max(float(source_token_payload_bits), 1.0),
        "beats_source_token_strict_margin": float(int(strict_improvement > float(SOURCE_TOKEN_STRICT_IMPROVEMENT))),
...
def run_profile(profile: str, seed: int = SEED) -> dict[str, Any]:
    ...
    engineering_pass = float(int(metrics["exact_reconstruction_success"] == 1.0 and metrics["random_label_exact_reconstruction_success"] == 1.0 and metrics["random_label_payload_incompressible"] == 1.0 and metrics["beats_source_token_strict_margin"] == 1.0 and strict_pass == 1.0 and payload_pass == 1.0 and controls_pass == 1.0 and overlaps["source_train_test_path_overlap_count"] == 0.0 and overlaps["source_train_test_hash_overlap_count"] == 0.0))
    return {
        "profile": profile,
        ...
        "source_subtoken_structure_block_codec_candidate": engineering_pass,
...
def main() -> int:
    ...
    metrics_path = output_dir / "local_100k_source_subtoken_structure_block_codec_metrics.json"
    ...
    record = build_run_record(
        ...
        parameters={"profile": profile, "seed": int(SEED), "decoder_bits": int(DECODER_BITS), "model_header_bits": int(MODEL_HEADER_BITS), "surface_contract_bits": int(SURFACE_CONTRACT_BITS), "subtoken_structure_header_bits": int(SUBTOKEN_STRUCTURE_HEADER_BITS), "token_limit": int(TOKEN_LIMIT)},
        seed_numpy=int(SEED),
        n_trials=int(summary[f"{SIMULATION_ID}_target_block_bytes"]),
        summary=summary,
        statistics={},
        trials=[],
        artifacts=[{"name": "local_100k_source_subtoken_structure_block_codec_metrics.json", "path": metrics_path}],
        warnings=["narrow source-code subtoken-structure block codec; target dictionary is charged; no nm, chat, knowledge, paid-compute, or broad breakthrough authorization"],
    )
