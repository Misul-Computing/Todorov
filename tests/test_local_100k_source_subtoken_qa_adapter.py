...
PREFIX = "local_100k_source_subtoken_qa_adapter"
...
def test_source_subtoken_qa_cell_answers_from_compressed_state() -> None:
    train, facts, source_block, profile = build_facts(2137, 512)
    cell = SourceSubtokenQAAdapterCell(train, facts, source_block, profile)
    answers = cell.answer_many(questions_for(facts))
    assert exact_success_count(facts, answers) == len(facts)
    assert cell.subtoken_codec_used == 1.0
...
def test_source_subtoken_qa_payload_beats_raw_scan_but_not_same_subtoken_scan() -> None:
    train, facts, source_block, profile = build_facts(2137, 512)
    cell = SourceSubtokenQAAdapterCell(train, facts, source_block, profile)
    ...
    assert cell.block_payload_bits == 244440
    assert raw["raw_best_standard_payload_bits"] == 262336


def test_source_subtoken_qa_summary_reports_product_and_limits() -> None:
    summary = build_summary("smoke", seed=2137)
    assert summary[f"{PREFIX}_engineering_pass"] == 1.0
    assert summary[f"{PREFIX}_subtoken_qa_product_candidate"] == 1.0
    ...
    assert summary[f"{PREFIX}_same_subtoken_content_scan_beaten"] == 0.0
    assert summary[f"{PREFIX}_same_subtoken_content_scan_not_beaten"] == 1.0
    assert summary[f"{PREFIX}_adapter_strict_multiplier"] > summary[f"{PREFIX}_raw_executable_content_scan_multiplier"]
    assert summary[f"{PREFIX}_adapter_strict_multiplier"] <= summary[f"{PREFIX}_same_subtoken_content_scan_multiplier"]
    assert summary[f"{PREFIX}_adapter_strict_multiplier"] > 24.0
    assert summary[f"{PREFIX}_paper_surface_strict_multiplier"] > 23.8
...
def test_source_subtoken_qa_registry_entry() -> None:
    ...
    assert dict(spec.minimum_summary_values)[f"{PREFIX}_paper_surface_strict_multiplier"] == 23.8
    assert dict(spec.maximum_summary_values)[f"{PREFIX}_same_subtoken_content_scan_beaten"] == 0.0
