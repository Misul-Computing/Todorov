...
## current phase: neural model master-plan research phase (2026-04-26)
...
paid compute remains paused. it can return only after broad lane research, oracle compression analysis, a tiny trainable mirror clears cpu controls, telemetry is complete, canonical state is prosecutor-clean, and one selected hypothesis is explicitly approved in `neuroloc/wiki/PROJECT_PLAN.md`.
