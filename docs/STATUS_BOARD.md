...
the six lanes are cellular state storage, operation-preserving compression, memory/addressing/replay/imagination, 3d world model and physics, trainability and evaluation, and project infrastructure/curriculum/paper operations. the first no-paid cellular/local-state gap map is now `neuroloc/wiki/synthesis/cellular_state_storage_gap_map.md`: it frames local cellular state as fast carry, eligibility or write-permission trace, bounded output exposure, and slow stability or control state, with strict proof gates before code. the next no-paid work is the first cellular mechanism dossier plus oracle compression bounds. the paper-spine pair remains active: `neuroloc/wiki/synthesis/neural_model_paper_spine.md` and `neuroloc/wiki/synthesis/oracle_compression_analysis_plan.md`.
