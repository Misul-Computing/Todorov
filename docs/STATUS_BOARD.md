...
the six lanes are cellular state storage, operation-preserving compression, memory/addressing/replay/imagination, 3d world model and physics, trainability and evaluation, and project infrastructure/curriculum/paper operations. the first no-paid cellular/local-state gap map is `neuroloc/wiki/synthesis/cellular_state_storage_gap_map.md`, the first mechanism dossier is `neuroloc/wiki/synthesis/neural_model_dossier_eligibility_gated_local_commit.md`, the first mechanism-specific symbolic contract is `neuroloc/wiki/synthesis/neural_model_symbolic_contract_eligibility_gated_local_commit.md`, the implemented symbolic/oracle package is `neuroloc/wiki/tests/eligibility_gated_local_commit_test_material.md`, the first oracle compression result is `neuroloc/wiki/tests/oracle_compression_analysis_results.md`, the frontier split is `neuroloc/wiki/synthesis/oracle_compression_frontier_split.md`, the first narrow learned-codec proof package is `neuroloc/wiki/synthesis/neural_model_dossier_compression_under_bit_budget_codec.md`, the tiny local mirror contract is `neuroloc/wiki/synthesis/neural_model_tiny_mirror_contract_compression_under_bit_budget.md`, and the first local mirror code surface is `neuroloc/wiki/tests/compression_under_bit_budget_mirror.md`. the subq / selective sparse attention related-work boundary is now recorded at `neuroloc/wiki/synthesis/content_routed_sparse_read_prior.md`, and the broader six-lane literature pass is consolidated at `neuroloc/wiki/synthesis/neural_model_related_work_pressure_matrix.md`: these validate content-dependent routing, functional context, task-relative compression, oracle localization, and exact-state simulator gates as prior-art pressure, but they do not prove memory-object compression or authorize paid compute. the current mirror is demoted as compression evidence and retained as a source-selection/useful-bit diagnostic: content-routed sparse read solves the repaired source-pair task at joint/state/action success 1.0 by selecting two legal observation records, with source-selection recall 1.0, next-source-selection recall 1.0, false-source-selection rate 0.0, 40 committed bits, 1.3x compression versus verbatim, and within-budget 0.0; matched-budget sparse read commits 20 bits, stays within budget, and fails. a distributed-evidence probe is now implemented with no commit markers and four legal evidence fragments: uncapped sparse read solves it at 80 bits, while matched-budget sparse read fails at 20 bits. the learned codec remains negative: train joint success 1.0, validation joint success 0.0, test joint success 0.125 in the latest smoke run, learned compression ratio versus verbatim 2.74x, learned-minus-sparse-read joint gap -0.875, source event observed rate 1.0, required source fields visible rate 1.0, source state reconstructable rate 1.0, visible-source-codec joint/state/action success 1.0, oracle-code/learned-decoder train 1.0 but validation/test 0.0, and no engineering or paper-track pass. the next no-paid work is a learned compact-code result on distributed evidence, starting with oracle-code/learned-decoder generalization. the paper-spine pair remains active: `neuroloc/wiki/synthesis/neural_model_paper_spine.md` and `neuroloc/wiki/synthesis/oracle_compression_analysis_plan.md`.
