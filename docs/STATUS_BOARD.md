...
## current phase: neural model master-plan research phase (2026-05-08)
...
the six lanes are cellular state storage, operation-preserving compression, memory/addressing/replay/imagination, 3d world model and physics, trainability and evaluation, and project infrastructure/curriculum/paper operations. the first no-paid cellular/local-state gap map is `neuroloc/wiki/synthesis/cellular_state_storage_gap_map.md`, the first mechanism dossier is `neuroloc/wiki/synthesis/neural_model_dossier_eligibility_gated_local_commit.md`, the first mechanism-specific symbolic contract is `neuroloc/wiki/synthesis/neural_model_symbolic_contract_eligibility_gated_local_commit.md`, the implemented symbolic/oracle package is `neuroloc/wiki/tests/eligibility_gated_local_commit_test_material.md`, the first oracle compression result is `neuroloc/wiki/tests/oracle_compression_analysis_results.md`, the frontier split is `neuroloc/wiki/synthesis/oracle_compression_frontier_split.md`, the first narrow learned-codec proof package is `neuroloc/wiki/synthesis/neural_model_dossier_compression_under_bit_budget_codec.md`, the tiny local mirror contract is `neuroloc/wiki/synthesis/neural_model_tiny_mirror_contract_compression_under_bit_budget.md`, the first local compact-state mirror is `neuroloc/wiki/tests/compression_under_bit_budget_mirror.md`, the first constrained message-response bridge is `neuroloc/wiki/tests/language_grounded_state_density_mirror.md`, the first dataset-grounded local v1 language-state gate is `neuroloc/wiki/tests/local_v1_language_model.md`, and the first integrated 10k local chat gate is `neuroloc/wiki/tests/local_10k_chat_model.md`. the subq / selective sparse attention related-work boundary is now recorded at `neuroloc/wiki/synthesis/content_routed_sparse_read_prior.md`, and the broader six-lane literature pass is consolidated at `neuroloc/wiki/synthesis/neural_model_related_work_pressure_matrix.md`: these validate content-dependent routing, functional context, task-relative compression, oracle localization, and exact-state simulator gates as prior-art pressure, but they do not prove memory-object compression or authorize paid compute. the original source-pair mirror is demoted as compression evidence and retained as a source-selection/useful-bit diagnostic. the 10k-scale local data-heavy compact-state gate clears four heldout axes and three seeds with 9,792 parameters, 24,576 generated train records, and 19 committed bits. the structured language-grounded bridge trains a 3,847-parameter local model from generated text observations and text queries across four heldout axes and two seeds: 16,384 train records, 768 validation records, 768 test records, minimum test joint/state/action success 1.0, 19 committed bits, useful operation success per committed bit 0.05263157894736842, and matched-budget sparse read at 20 bits with 0.0 max joint success. the parser-resistant learned token-count gate is negative: randomized prompts remove stable field prefixes, shuffle event order, inject irrelevant clauses, report parser/schema cost, and force a 1,779-parameter learned token-count encoder through bounded local state; it reaches minimum test joint/state success 0.0 while uncapped sparse read remains 1.0. the event-binding parser-supported foundation keeps the randomized message surface but preserves event segments as bounded local state before answering: minimum test joint/state/action success 1.0, 19 state bits plus 37 parser/schema bits for 56 accounted bits, matched-budget sparse read 0.0 at 20 bits, zero-state joint max 0.0, shuffled-state joint max 0.020833333333333332, and parser-supported foundation pass 1.0. the typed trainable segment binder then clears the same local gate with 8,856 parameters, 16,384 train records, 768 validation records, minimum test joint/state/action success 0.9583333333333334, field floor 0.9583333333333334, trainable useful operation success per accounted bit 0.01711309523809524, zero-state joint max 0.0, shuffled-state joint max 0.020833333333333332, and engineering pass 1.0. the local v1 language-state gate trains on dataset-derived memory records rather than next-token prediction, uses 2,720 trainable routing parameters over 32 local records, reaches minimum joint/state/action/provenance success 1.0, zero-state and shuffled-state joint success 0.0, 28 accounted bits, and engineering pass 1.0. the current `local_10k_chat_model` artifact extends that foundation into a runnable constrained chat surface with 7,968 trainable parameters, 32 local records, 61 accounted bits, grounded response success 0.96875, unknown refusal success 1.0, route-disabled grounded success 0.03125, shuffled-route grounded success 0.0, memory update success 1.0, short-term context success 1.0, targeted replay success 1.0, branch-state success 1.0, final dialogue joint success 1.0, command-line prompt mode, and suite pass. this is a constrained grounded 10k chat foundation, not arbitrary chat, not solved compression, not open-ended imagination, and not full-scale neural-model v1. the next no-paid work is larger local data ingestion, longer grounded dialogue, provenance-preserving rewrites, fair sparse-read baselines, factor-heldout query forms, zero-state, shuffled-state, random-replay, and useful-density controls. the paper-spine pair remains active: `neuroloc/wiki/synthesis/neural_model_paper_spine.md` and `neuroloc/wiki/synthesis/oracle_compression_analysis_plan.md`.
